...
static int symwalk_cb(void *data, const char *name, struct module *mod, unsigned long addr)
{
    ...
    if (strcmp(name, "vread") == 0)
    {
        if (vread_p != NULL)
        {
            printk(KERN_INFO "[mykmem] found two vread, terminating\n");
            return -EFAULT;
        }
        printk(KERN_INFO "[mykmem] found vread at %lx \n", addr);
    ...
    if (strcmp(name, "vwrite") == 0)
    {
        if (vwrite_p != NULL)
        {
            printk(KERN_INFO "[mykmem] found two vwrite, terminating\n");
            return -EFAULT;
        }
        printk(KERN_INFO "[mykmem] found vwrite at %lx \n", addr);
...
static int __init chr_dev_init(void)
{
    ...
    int ret, minor;
	printk(KERN_INFO "[mykmem] MyKmem is loading\n");
    ret = kallsyms_on_each_symbol(symwalk_cb, NULL);
    if (ret)
        return ret;
    
	majorNum = register_chrdev(0, DEVICE_NAME, &kmem_fops);
	if (majorNum < 0){
		printk(KERN_ALERT "[mykmem] unable to get major for MyKmem\n");
		return majorNum;
	}
	...
	if (IS_ERR(kmemClass)){
		...
		printk(KERN_ALERT "[mykmem] Failed to register kmem class\n");
		return PTR_ERR(kmemClass);
	}

	
	//printk(KERN_INFO "[mykmem] Registering Minors\n");
	kmemClass->devnode = mem_devnode;
	for (minor = 1; minor < ARRAY_SIZE(devlist); minor++) {
		if (!devlist[minor].name)
			continue;

	    printk(KERN_INFO "[mykmem] Trying to Register %s with %d\n", devlist[minor].name, minor);
		tmpDev = device_create(kmemClass, NULL, MKDEV(majorNum, minor),
			      NULL, devlist[minor].name);
        if (IS_ERR(tmpDev)){
            class_destroy(kmemClass);
            unregister_chrdev(majorNum, DEVICE_NAME);
            printk(KERN_ALERT "[mykmem] Failed to register minor %d for %s\n", minor, devlist[minor].name);
    ...
    printk(KERN_INFO "[mykmem] executable page at: %llx\n", (unsigned long long)execplay);
    printk(KERN_INFO "[mykmem] MyKmem device created\n");
    return 0;
}
...
static void __exit kmem_exit(void){
    ...
    printk(KERN_INFO "[mykmem] Goodbye from the LKM!\n");
